...
// 민우가 구매한 로또 번호를 담은 배열 lottos, 당첨 번호를 담은 배열 win_nums가 매개변수로 주어집니다. 이때, 당첨 가능한 최고 순위와 최저 순위를 차례대로 배열에 담아서 
// return 하도록 solution 함수를 완성해주세요.


// [44, 1, 0, 0, 31, 25]	[31, 10, 45, 1, 6, 19]	         결과 :[3, 5]
// [0, 0, 0, 0, 0, 0]	    [38, 19, 20, 40, 15, 25]	     결과 :[1, 6]
// [45, 4, 35, 20, 3, 9]	[20, 9, 3, 45, 4, 35]	         결과 :[1, 1]
...
function solution(lottos, win_nums) {
    ...
    let win = lottos.filter(element => win_nums.includes(element)).length;
    let fail = lottos.filter(element => 0 === element).length 

    let rank = [0,0]

    let basket = { 6: 1 , 5:2, 4: 3, 3: 4, 2:5 , 1: 6 , 0: 6}

    if(basket.hasOwnProperty(win)){
